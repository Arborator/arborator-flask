...
def delete(project):
    ...
    related_metafeature = MetaFeature.query.filter_by(project_id=project.id).delete()
    db.session.commit()
...
def remove_feature(project, featurevalue):
    f = Feature.query.filter_by(project_id=project.id, value=featurevalue).first()
    db.session.delete(f)
    db.session.commit()
...
def remove_metafeature(project, featurevalue):
    f = MetaFeature.query.filter_by(project_id=project.id, value=featurevalue).first()
    db.session.delete(f)
    db.session.commit()