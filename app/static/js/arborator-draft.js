...
this.ArboratorDraft = function(visuMode = 0, reverse = false) {
	...
	$('sentence').each(function(){
		...
		sentencenr++;
		var data=conllNodesToTree($(this).children('conll').html().trim());
		$(this).prepend( $("<sentencetext></sentencetext>").attr('id', 'xxx').text(sentencenr+". "+data.sentence ) );
	...
	$( "user" ).click(function(){
		...
		// data has 3 attributes sentence, tree and uextra
		var data=conllNodesToTree($(this).next('conll').html().trim());
		
		// create a div under user to store the svg
		var divsvgbox  = d3.select(this).append("div").attr("class", "svgbox");
		...
		clicked = 1;
			...
			// remove the div with the tree
			d3.select(this).select("div.svgbox").remove();
			clicked = 0;
		}
...
function progressiveReadConll() {
	...
	log(7777,conllLoop[0],typeof(conllLoop[0]))
	var i = 0;
...
function progressiveReadSentences() {
	return;
	// draw each conll tags progressively (only conll tags)
// 	log(1111,d3.selectAll('sentence'));
// 	log(d3.selectAll('sentence')['_groups']);
// 	var sentenceLoop = d3.selectAll('sentence')['_groups'][0]; // need to go out d3js to load it progressively
// 	log(sentenceLoop);
// 	var i = 0;
// 	function waitBetweenElements(range) {
// 		readConllTimeout = setTimeout(function () {
// 
// 			drawConll(sentenceLoop[i]);
// 			i++;
// 			if (i < range) {
// 				waitBetweenElements(range);
// 			}
...
// 	}
// 
// 	waitBetweenElements(sentenceLoop.length);
}


function progressiveReadInsideConll(trees, pnode) {
	// draw each tree INSIDE one unique conll progressively
	var iWait = 0;
	function waitBetweenElements(range) {
...
// 		progressiveReadConll(); // progressive draw
		progressiveReadSentences(); // progressive draw
	}else{
...
function conllNodesToTree(treeline) {
	...
	// returns a dictionary that gives the analysis of the treeline
	// log(typeof(treeline));
...
function getSVGPath(startPoint,endPoint,computedStyle) {
	...
	// un truc du genre startPoint = d3.select(this).select("div.svgbox").select("g").selectAll("text")[0]
