...
# local imports
from . import project
from ...models.models import ProjectAccess, SampleRole
    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            if not current_user.super_admin:  # super_admin are always admin even if it's not in the table
                ...
                # if project_access is None or project_access.access_level.code < access_level: abort(403)
...
def project_info(project_name):
    ...
    infos: {
                            ...
                            admins : [],
                            samples: [
                                    { sample_name: 'P_ABJ_GWA_10_Steven.lifestory_PRO', sentences: 80, tokens: 20, averageSentenceLength: 12.6, roles :{validators: [], annotators: []}, treesFrom: ['parser', 'tella', 'j.D'], exo: 'percentage'}, 
    """
    project_infos = project_service.get_infos(
        project_name, current_user)  # FIX : handles anonymous user
...
def update_project_settings(project_name):
    ...
    for a, v in request.json.items():
        if a == "shownfeatures":
            ...
            reply = json.loads(grew_request("updateProjectConfig", current_app, data={
                               "project_id": project_name, "config": json.dumps(v)}))
            if reply["status"] != "OK":
                abort(400)
            print("reply", reply)
        elif a == "showAllTrees":
...
def project_userrole_add(project_name, target_role):
    ...
    if user:
        ...
        access_level_dict = {v: k for k, v in dict(
            ProjectAccess.ACCESS).items()}
        if pa:
            pa = access_level_dict[target_role]
        else:
            project_service.create_add_project_access(
                user.id, project.id, access_level_dict[target_role])
    project_infos = project_service.get_settings_infos(
        project_name, current_user)
    if project_infos == 403:
        abort(403)
...
def project_userrole_update_many(project_name, target_role) -> Response:
    ...
    new_access_level = access_level_dict[target_role]
...
def project_defaultusertrees_add(project_name):
    ...
    if 'robot' in selected:
        ...
        project_service.add_default_user_tree(
            project, r.id, r.username, robot=True)
    else:
        # user = user_service.get_by_id(request.json.get("user_id"))
        user = user_service.get_by_id(selected['id'])
        if user:
            project_service.add_default_user_tree(
                project, user.id, user.username)
    project_infos = project_service.get_settings_infos(
        project_name, current_user)
    if project_infos == 403:
        abort(403)
...
def delete_project(project_name):
    ...
    p_access = 0
    if pa == 0:
        print('unauthorized, pa 0, error on crreation no access set')
        project_service.delete(project)
    else:
        p_access = project_service.get_project_access(
            project.id, current_user.id)
    if p_access >= 2 or current_user.super_admin:
        project_service.delete(project)
    else:
        print("p_access to low for project {}".format(project.project_name))
        abort(403)
...
def search_project(project_name):
    ...
    reply = json.loads(grew_request("searchPatternInGraphs", current_app, data={
                       "project_id": project.project_name, "pattern": pattern}))
    if reply["status"] != "OK":
        abort(400)
    trees = {}
    ...
    for m in reply["data"]:
        if m['user_id'] == '':
            abort(409)
        conll = json.loads(grew_request("getConll", current_app, data={
                           "sample_id": m["sample_id"], "project_id": project.project_name, "sent_id": m["sent_id"], "user_id": m['user_id']}))
        if conll["status"] != "OK":
            abort(404)
        conll = conll["data"]
...
def tryRule_project(project_name):
    ...
    reply = json.loads(grew_request("tryRule", current_app, data={
                       "project_id": project.project_name, "pattern": pattern, "commands": rewriteCommands}))
    print(8989, reply)
    if reply["status"] != "OK":
        if 'message' in reply:
            resp = jsonify(
    ...
    for m in reply["data"]:
        ...
        if 'sentence' not in trees['sample_id']['sent_id']:
            trees['sample_id']['sent_id']['sentence'] = conll3.conll2tree(
                m['conll']).sentence()
...
def sample_upload(project_name):
    ...
    if robot_active == 'true':
        ...
        import_user = request.form.get(
            "import_user", "")  # TODO : change import_user
...
def project_create_upload(project_name):
    ...
    if fichiers:
        ...
        # samples  = project_service.get_samples(project_name)
        for f in fichiers:
            project_service.upload_project(
                f, project_name, import_user, reextensions=reextensions)
...
def project_update_config(project_name):
    ...
    if not data.get("relations"):
        data["relations"] = [["subj", "comp", "vocative", "det", "dep", "mod", "conj", "cc", "parataxis", "fixed", "flat", "compound", "discourse",
                              "dislocated", "goeswith", "orphan", "punct", "root"], [":aux", ":caus", ":cleft", ":pred", ":appos"], ["@comp", "@mod", "@subj", "@dep", "@det"]]
    if not data.get("cat"):
        data["cat"] = ["ADJ", "ADP", "ADV", "AUX", "CCONJ", "DET", "INTJ",
                       "NOUN", "NUM", "PART", "PRON", "PROPN", "PUNCT", "SCONJ", "VERB", "X"]
    js = json.dumps(data)
    resp = Response(js, status=200,  mimetype='application/json')
    return resp
...
def sample_export(project_name):
    ...
    for sample_name in sample_names:
        ...
        if reply.get("status") == "OK":
            ...
            sample_tree = json.loads(
                project_service.servSampleTrees(reply.get("data", {})))
            sample_content = project_service.sampletree2contentfile(
                sample_tree)

            for sent_id in sample_tree:

                last = project_service.get_last_user(
    ...
    memory_file = project_service.contentfiles2zip(
...
def sampleusers(project_name, sample_name):
    ...
    if not project:
        abort(404)
    sampleusers = project_service.get_samples_roles(
        project.id, sample_name, json=True)
...
def addRole2Sample(project_name, role):
    ...
    if 'sample_name' in req:
        if not req['sample_name'] in samples["samples"]:
            abort(404)
        possible_roles = [x[0] for x in project_service.get_possible_roles()]
        roleInt = [r[0] for r in project_service.get_possible_roles()
                   if r[1] == role][0]
        user = user_service.get_by_username(req['username'])
        if not user:
            abort(400)
        project_service.add_or_delete_sample_role(
            user, req['sample_name'], req['project_name'], roleInt, False)
        sample = project_service.get_sample(
            req['sample_name'], req['project_name'])
        res = sample
    js = json.dumps(res)
    resp = Response(js, status=200,  mimetype='application/json')
    return resp
...
def removeRole2Sample(project_name, role):
    ...
    if 'sample_name' in req:
        ...
        if not user:
            abort(400)
        project_service.add_or_delete_sample_role(
            user, req['sample_name'], req['project_name'], roleInt, True)
        sample = project_service.get_sample(
            req['sample_name'], req['project_name'])
        res = sample
    js = json.dumps(res)
    ...
    for u, r in request.json.items():
        ...
        if r:
            if r not in possible_roles:
                abort(400)
            project_service.create_add_sample_role(
                u, sample_name, project.id, r)
        else:
            sr = SampleRole.query.filter_by(
                project_id=project.id, sample_name=sample_name, user_id=u).first()
            if sr:
    ...
    if data:
        ...
        for tree in trees:
            ...
            if resp["status"] != "OK":
                if "data" in resp:
                    response = jsonify(
                        {'status': 400, 'message': str(resp["data"])})
                else:
                    response = jsonify(
                        {'status': 400, 'message': 'You idiots!...'})
...
def get_relation_table(project_name):
    ...
    reply = grew_request(
        ...
        data={'project_id': project_name, "pattern": 'pattern { e: GOV -> DEP}',
              "clusters": ["e; GOV.upos; DEP.upos"]}
    )
                ...
                for elt in vvv:
                    if table_type == 'user':
                        if elt["user_id"] != current_user.username:
                            ...
                            conll = elt.get("conll")
                            trees = project_service.formatTrees_new(
                                elt, trees, conll)
                    else:
                        conll = elt.get("conll")
                        trees = project_service.formatTrees_new(
                    ...
                    for samp in trees:
                        for sent in trees[samp]:
                            if current_user.username in trees[samp][sent]["conlls"]:
                                trees[samp][sent]["conlls"] = {
                                    current_user.username: trees[samp][sent]["conlls"][current_user.username]}
                                trees[samp][sent]["matches"] = {
                                    current_user.username: trees[samp][sent]["matches"][current_user.username]}
                            else:
                                last = project_service.get_last_user(
                                    trees[samp][sent]["conlls"])
                                trees[samp][sent]["conlls"] = {
...
def commit_sample(project_name, sample_name, commit_type):
    ...
    if reply.get("status") == "OK":
        ...
        for sent_id in sample:
            ...
            if commit_type == 'all':
                ...
                if current_user.username in sample[sent_id]:
                    ...
                    content[current_user.username] = content.get(
                        current_user.username, []) + [conll]
        ...
        # user-based mode
            ...
            user_repo = github_service.get_user_repository(
            ...
            resp = github_service.exists_sample(
                current_user.username, project_name, sample_name+suffix)

            if resp.status_code == 200:
                print("updating sample - commit type : {}".format(commit_type))
                sha = json.loads(resp.content.decode())["sha"]
                data = {'sha': sha, 'content': content, 'message': 'updating sample', 'author': {
                    "name": current_user.username, "email": "unknown"}}
            else:
                print(resp.status_code)
                print("new sample - commit type : {}".format(commit_type))
                data = {'content': content, 'message': 'uploading a new sample', 'author': {
                    "name": current_user.username, "email": "unknown"}}

            resp = github_service.make_commit(
                user_repo, data, project_name, sample_name+suffix)
            print(resp.status_code)
        ...
        # admin mode : commit all trees in separate files
            ...
            for username in content:
                ...
                conll_content = base64.b64encode(
                    conll_content.encode("utf-8")).decode("utf-8")
                ...
                if resp.status_code == 200:
                    print("updating sample - commit type : {}".format(commit_type))
                    sha = json.loads(resp.content.decode())["sha"]
                    data = {'sha': sha, 'content': conll_content, 'message': 'updating sample', 'author': {
                        "name": current_user.username, "email": "unknown"}}
                else:
                    print(resp.status_code)
                    print("new sample - commit type : {}".format(commit_type))
                    data = {'content': conll_content, 'message': 'uploading a new sample', 'author': {
                        "name": current_user.username, "email": "unknown"}}
    ...
    if installation_id:
        for sample_name in sample_names:
            ...
            if exit_code == 204:
                ...
                resp = jsonify({'status': exit_code, 'message': 'No trees were found for user {}, please make some trees before comitting the samples.'.format(
                    current_user.username)})
...
def pull_sample_and_save_on_grew(github_user, project_name, sample_name, tree_user):
    resp = github_service.get_sample(
        github_user, project_name, sample_name+"_"+tree_user)
    if resp.status_code == 404:
        resp = Response({"message": "This sample is absent from your github storage which makes pulling impossible."},
        ...
        reply = grew_request(
            ...
            data={'project_id': project_name,
                  'sample_id': sample_name, 'user_id': tree_user},
...
def pull(project_name):
    ...
    if installation_id:
        for sample_name in sample_names:
            ...
            if pull_type == 'user':
                print("!!")
                status = pull_sample_and_save_on_grew(
                    current_user.username, project_name, sample_name, current_user.username)
                print(current_user.username, status)
            elif pull_type == 'all':
                print("**")
                users = github_service.get_all_users(
                    current_user.username, project_name, sample_name)
                for u in users:
                    status = pull_sample_and_save_on_grew(
                        current_user.username, project_name, sample_name, u)
                    print(u, status)
        ...
        # TODO mettre un petit message pour dire qu'il faut se connecter via github + donner permissions
        resp: Response = jsonify({'status': status, 'message': message})
        resp.status_code = status
        return resp
...
def getLexicon(project_name):
    ...
    reply = json.loads(grew_request("getLexicon", current_app, data={
                       "project_id": project_name, 'sample_ids': json.dumps(sample_names)}))
    for i in reply['data']:
...
def transformation_grew(project_name):
    ...
    for i in lexicon['data']:
        ...
        comp += 1
        patterns.append(
            project_service.transform_grew_get_pattern(line1, dic, comp))
        resultat = project_service.transform_grew_verif(line1, line2)
        co, without_traits = (project_service.transform_grew_get_commands(
            resultat, line1, line2, dic, comp))
        commands.append(co)
        if without_traits != '':
            without = without+without_traits
    patterns[
        0] = '% click the button \'Correct lexicon\' to update the queries\n\npattern { '+patterns[0][0:]
    ...
    resp = jsonify({'patterns': patterns_output,
                    'commands': commands_output, 'without': without})
...
def addValidator(project_name):
    ...
    AB_Ok = []
    AB_Diff = []
    list_types = {"In the two dictionaries with the same information": AB_Ok,
                  "In the two dictionaries with different information": AB_Diff, "Only in the old dictionary": A, "Only in the imported dictionary": B}
