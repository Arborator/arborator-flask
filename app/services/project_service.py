...
def create_add_project_access(user_id, project_id, access_level):
    ...
    pa = ProjectAccess(userid=user_id, projectid=project_id,
...
def delete_by_name(project_name):
    ...
    grew_request('eraseProject', current_app, data={
...
def get_settings_infos(project_name, current_user):
    ...
    # config from grew
    reply = json.loads(grew_request("getProjectConfig",
                                    current_app, data={"project_id": project_name}))
    if reply["status"] != "OK":
        abort(400)
    ...
    defaultUserTrees = [u.as_json()
                        for u in project_dao.find_default_user_trees(project.id)]
...
def get_project_config(project_name):
    ...
    '''
    reply = grew_request('getProjectConfig', current_app,
...
def get_hub_summary():
    ...
    for project in projects:
        ...
        projectJson = project.as_json(
            include={"admins": admins, "guests": guests})
...
def get_project_treesfrom(project_name):
    ...
    project = project_dao.find_by_name(project_name)
    reply = grew_request('getSamples', current_app, data={
                         'project_id': project_name})
    js = json.loads(reply)
    data = js.get("data")
    treesFrom = list()
...
def create_add_sample_role(user_id, sample_name, project_id, role):
    ...
    if existing_role:
        project_dao.delete_sample_role(existing_role)
    new_sr = SampleRole(userid=user_id, samplename=sample_name,
...
def create_empty_project(project_json, creator):
    ...
    new_project = grew_request('newProject', current_app, data={
                               'project_id': project_json['name']})
    ...
    project = Project(projectname=project_json['name'], description=project_json['description'], visibility=project_json['visibility'],
                      show_all_trees=project_json['showAllTrees'], exercise_mode=project_json['exerciseMode'])
...
def delete_sample(project_name, project_id, sample_name):
    ...
    grew_request('eraseSample', current_app, data={
                 'project_id': project_name, 'sample_id': sample_name})
    related_sample_roles = project_dao.delete_sample_role_by_project(
        project_id)
...
def get_samples_roles(project_id, sample_name, json=False):
    ...
    sampleroles = SampleRole.query.filter_by(
        projectid=project_id, samplename=sample_name).all()
    if json:
...
def upload_sample(fileobject, project_name, import_user, reextensions=None, existing_samples=[]):
    ...
    if sample_name not in existing_samples:
        ...
        print('========== [newSample]')
        reply = grew_request('newSample', current_app, data={
                             'project_id': project_name, 'sample_id': sample_name})
        print('reply = ', reply)
    ...
    tmpfile = add_or_keep_timestamps(
    ...
    with open(tmpfile, 'rb') as inf:
        ...
        if import_user:
            reply = grew_request(
                ...
                data={'project_id': project_name,
                      'sample_id': sample_name, "user_id": import_user},
    ...
    if reply.get("status") == "OK":
        ...
        error_sent_id = reply.get('message', {}).get(
            'Conllx_error:').get("sent_id", "")
        ...
        for samplename, sample in zip(samplenames, sampletrees):
            for fuser, filecontent in sample.items():
                data = zipfile.ZipInfo(
                    '{}.{}.conllu'.format(samplename, fuser))
...
def formatTrees_new(m, trees, conll):
    ...
    if sent_id not in trees[sample_name]:
        t = conll3.conll2tree(conll)
        s = t.sentence()
        trees[sample_name][sent_id] = {"sentence": s, "conlls": {
            user_id: conll}, "matches": {user_id: [{"edges": edges, "nodes": nodes}]}}
    else:
        ...
        trees[sample_name][sent_id]["matches"][user_id] = trees[sample_name][sent_id]["matches"].get(
            user_id, [])+[{"edges": edges, "nodes": nodes}]
    ...
    for e in resultat:
        if e == 4:  # si traits sont différents
            ...
            if ligne2[e] != "_":
                ...
                traits_a_supprimer = transform_grew_traits_corriges(
                    ligne1[e], comp)
